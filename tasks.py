...
tokens = [word.strip(''.join(punct)) for word in text.split()]
...
# Your code here:
# -----------------------------------------------
token_to_id = {word: id for id, word in enumerate(tokens)}
...
# Your code here:
# -----------------------------------------------
id_to_token = {word: id for word, id in enumerate(tokens)}
...
def make_vocabulary_map(documents: list) -> tuple:
    ...
    for doc in documents:
        all_tokens.extend(tokenize(doc))  # flatten unique tokens from each doc

    # Make all tokens unique across documents
    unique_tokens = sorted(set(all_tokens))

    token_to_id = {word: idx for idx, word in enumerate(unique_tokens)}
